...
namespace SASSTS.Business.FileProcess
{
    public static class PathUtil
    {
        public static string GenerateFileNameFromBase64File(string base64Image, string productName)
        {
            var date = DateTime.Now;
            var extension = GetExtensionFromBase64String(base64Image);
            var fileName = $"{date.Day}_{date.Month}_{date.Year}_{date.Hour}_{date.Minute}_{date.Second}_{date.Millisecond}_{productName}{extension}";
            return fileName;
        }
